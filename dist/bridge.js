...
(() => {
    ...
    const proxyInstances = new Set();
    function createProxyCtor() {
        function ProxyCtor(...ctorArgs) {
            if (!(this instanceof ProxyCtor))
                throw new Error('JoyCon2 must be called with new');
            ...
            const enqueueTask = (type, args, maybeResolve, maybeReject) => {
            ...
            // proxy methods
            this.connect = (...args) => {
                if (this.__real && typeof this.__real.connect === 'function')
                    return this.__real.connect(...args);
                return enqueueTask('connect', args, null, null);
            };
            this.disconnect = (...args) => {
                if (this.__real && typeof this.__real.disconnect === 'function')
                    return this.__real.disconnect(...args);
                return enqueueTask('disconnect', args);
            };
            this.onUpdate = (cb) => {
                if (this.__real && typeof this.__real.onUpdate === 'function')
                    return this.__real.onUpdate(cb);
        ...
        try {
            Object.defineProperty(ProxyCtor, 'name', { value: 'JoyCon2' });
        }
        catch (e) { }
        return ProxyCtor;
    }
    function bindRealConstructor(RealCtor) {
        if (typeof RealCtor !== 'function')
            return;
        // set canonical global
        try {
            window.JoyCon2 = RealCtor;
        }
        catch (e) {
            // ignore
        }
